...
void getdata(pid_t child, long addr, char *str, int len){
	...
	for(int i = 0; i < len; i += long_size) {
		buf.val = ptrace(PTRACE_PEEKDATA, child, (void*)(addr + i), 0);
		if(i + long_size <= len) {
			memcpy(str + i, buf.s, long_size);
		} else {
			memcpy(str + i, buf.s, len - i);
		}
	}
}
...
void putdata(pid_t child, long addr, char *str, int len){
	...
	for(int i = 0; i < len; i += long_size) {
		if(i + long_size <= len) {
			memcpy(buf.s, str + i, long_size);
		} else {
			int off;
			for(off = 0; i + off < len; off++)
				buf.s[off] = str[i + off];
			for(; off < long_size; off++)
				buf.s[off] = 0;
		}
		ptrace(PTRACE_POKEDATA, child, (void*)(addr + i), buf.val);
	}
}
...
//			fprintf(stderr, "syscall %d we want %d\n", orig_rax, SYS_write);
			if(orig_rax == SYS_write) {
				if(toggle == 0) {
					 ...
					 **/
					params[0] = ptrace(PTRACE_PEEKUSER, child, 8 * RDI, 0);
					params[1] = ptrace(PTRACE_PEEKUSER, child, 8 * RSI, 0);
					params[2] = ptrace(PTRACE_PEEKUSER, child, 8 * RDX, 0);
					str = (char *)calloc((params[2]+1), sizeof(char));
					getdata(child, params[1], str,
							params[2]);
					reverse(params[2], str);
